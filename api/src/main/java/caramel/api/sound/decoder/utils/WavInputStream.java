    ...
    public boolean open() {
        try {
            if (read() != 'R' || read() != 'I' || read() != 'F' || read() != 'F') {
                Debug.logError("RIFF header not found: " + file.getPath());
                return false;
            }
            skipFully(4);

            if (read() != 'W' || read() != 'A' || read() != 'V' || read() != 'E') {
                Debug.logError("Invalid wave file header: " + file.getPath());
                return false;
            }
            ...
            if (type != 1) {
                ...
                Debug.logError("WAV files must be PCM, unsupported format: " + name + " (" + type + ")");
                return false;
            }

            channels = read() & 0xff | (read() & 0xff) << 8;
            if (channels != 1 && channels != 2) {
                Debug.logError("WAV files must have 1 or 2 channels: " + channels);
                return false;
            }
            ...
            if (bitsPerSample != 16) {
                Debug.logError("WAV files must have 16 bits per sample: " + bitsPerSample);
                return false;
            }
            ...
            Debug.logError("Error reading WAV file: " + file.getPath());
            return false;
        }
    }
